-- NexoPlay V10: reparación de soporte + despliegue de RPC de referidos VIP.
...
-- Asegura que el ticket conserve los datos pedidos por el formulario.
...
-- Índice para el hilo de mensajes por ticket.
create index if not exists idx_support_messages_ticket_created on public.support_messages(ticket_id, created_at);

-- Categorías del catálogo: elimina el 404/Warning si esta tabla aún no existe.
create table if not exists public.nexoplay_catalog_categories (
...
create index if not exists idx_nx_catalog_categories_active_sort on public.nexoplay_catalog_categories(active,sort_order,name);
alter table public.nexoplay_catalog_categories enable row level security;
drop policy if exists "Authenticated can read catalog categories" on public.nexoplay_catalog_categories;
create policy "Authenticated can read catalog categories" on public.nexoplay_catalog_categories for select to authenticated using (true);
drop policy if exists "Admin can write catalog categories" on public.nexoplay_catalog_categories;
create policy "Admin can write catalog categories" on public.nexoplay_catalog_categories for all to authenticated using (public.is_admin()) with check (public.is_admin());

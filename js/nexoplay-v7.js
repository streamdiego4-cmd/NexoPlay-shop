...
(function(){
  ...
  function tuneProfileLayout(){
    ...
    if(profile && profile.style.display!=='none')bg.dataset.nx7Mode='profile';
    else if(bg.dataset.nx7Mode==='profile')delete bg.dataset.nx7Mode;
  }
  ...
  function patchVipEntrypoints(){
    ...
    window.openNexoVip=fn;window.openVip=fn;window.renderVip=vipCenter;window.nx5RenderVip=vipCenter;
  }
  ...
  async function syncBusinessOrders(){
    ...
    try{
      ...
      data.forEach(r=>{
        const idx=orders.findIndex(o=>String(o.id)===String(r.order_code||r.id)&&String(o.ownerUserId)===String(u.id));
        if(idx<0)return;
        const o=orders[idx],items=Array.isArray(r.items)?r.items:[];
        const meta=items.find(x=>x&&((x.customer_name||x.customer_phone||x.sale_amount)>0||x.customer_name||x.customer_phone));
        if(meta&&!o.distributorCustomer)o.distributorCustomer={name:meta.customer_name||'',phone:meta.customer_phone||'',saleAmount:Number(meta.sale_amount||0)};
  ...
  function refreshSalesPanel(){
    ...
    syncBusinessOrders().finally(()=>{
      ...
      if(host)host.innerHTML=list||'<div class="distributor-empty">📦 Todavía no tienes ventas registradas.</div>';
  ...
  function sendDelivery(id){const s=currentBusinessOrders().find(x=>String(x.id)===String(id));if(!s||s.deliveryStatus!=='delivered'){window.showToast?.('La cuenta todavía no está entregada.');return}const d=s.distributorCustomer||{};const msg=`Hola ${d.name||'👋'} 😊\n\n📦 *Entrega de tu cuenta NexoPlay*\n\n🎬 Producto: ${saleText(s)}\n📧 Correo: ${s.deliveryEmail||'Pendiente'}\n🔑 Contraseña: ${s.deliveryPassword||'Pendiente'}\n👤 Perfil: ${s.deliveryProfile||'—'}\n🔢 PIN: ${s.deliveryPin||'—'}\n📅 Vencimiento: ${s.deliveryExpire||'—'}\n\nGracias por tu compra. ❤️`;openCustomerWhatsApp(d.phone,msg)}
  function sendExpiry(id){const s=currentBusinessOrders().find(x=>String(x.id)===String(id));if(!s||s.deliveryStatus!=='delivered'){window.showToast?.('Primero debe estar entregada la cuenta.');return}const d=s.distributorCustomer||{},left=daysLeft(s.deliveryExpire);const txt=left==null?'próximamente':left<0?'ya venció':left===0?'vence hoy':`faltan ${left} día${left===1?'':'s'}`;const msg=`Hola ${d.name||'👋'} 😊\n\n⏰ *Recordatorio NexoPlay*\nTu cuenta de *${saleText(s)}* ${txt}.\n📅 Vencimiento: ${s.deliveryExpire||'—'}\n\nSi deseas renovarla, comunícate con nosotros. ❤️`;openCustomerWhatsApp(d.phone,msg)}
  ...
  function patchProfile(){
    ...
    if(typeof original==='function'&&!original.__nx7){const f=async function(){const out=await original.apply(this,arguments);setTimeout(tuneProfileLayout,0);setTimeout(()=>window.nx5RefreshProfile?.(),10);return out};f.__nx7=true;window.showProfileView=f}
  }
  ...
  function install(){
    ...
    addLogoFallbacks();
    const mo=new MutationObserver(()=>{tuneProfileLayout();addLogoFallbacks();polishWallet();});
    mo.observe(document.body,{subtree:true,childList:true});
    setTimeout(()=>{patchCheckout();patchSalesPanel();patchProfile();patchVipEntrypoints();refreshSalesPanel()},350);
  }
